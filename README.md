...
Rolling Number is bundled with its MIT license in `Source/ROLLING-LICENSE`. Font licenses are in `Source/Fonts` and `Source/FREDOKA-LICENSE`. The `painted-*.jpg` wallpapers are generated by `Scripts/make-backgrounds.sh` (run it to regenerate them identically). The other background images were supplied for this app; their inclusion does not grant a separate license to reuse the artwork. No blanket license is asserted over third-party artwork.
