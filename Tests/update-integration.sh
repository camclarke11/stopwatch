...
built=$(/bin/zsh "$project/Scripts/git-update.sh" build "$PWD" "$commit" 1.0.1)
[[ "$(/usr/libexec/PlistBuddy -c 'Print CFBundleShortVersionString' "$built/Contents/Info.plist")" == 1.0.1 ]]
...
[[ "$(/usr/libexec/PlistBuddy -c 'Print CFBundleShortVersionString' "$fixture/installed/Stopwatch.app/Contents/Info.plist")" == 1.0.1 ]]
